...
interface MainHeroProps {
  data?: {
    ...
    industries?: ProfessionalItem[]; // Keep for backward compatibility
  };
}
...
export default function MainHero({ data }: MainHeroProps) {
  ...
  // Support both 'professionals' and 'industries' for backward compatibility
  const professionalsData = data?.professionals || data?.industries;
  const professionals = getProfessionals(professionalsData);
        ...
        <div className="relative mt-8 lg:mt-0">
          <div className="grid grid-cols-1 gap-4 pb-4 sm:grid-cols-2 sm:gap-5 md:gap-6 lg:flex lg:items-stretch lg:gap-6 lg:overflow-x-auto">
            {professionals.map((professional, index) => {
              return (
                <Link
                  key={`${professional.slug}-${index}`}
                  href={`/professionals-marketing-agency/${professional.slug}`}
                  className={cn(
