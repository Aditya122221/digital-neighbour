...
const Process2 = ({ data, processData }: Process2Props) => {
	...
	useEffect(() => {
		setMobileDotPositions(
			Array(steps.length).fill({ top: 0, opacity: 0.3 })
		)
	...
	useEffect(() => {
		const handleScroll = () => {
			...
			if (isMobile) {
				// Mobile scroll logic - determine active step by viewport position
				let currentStep = 0
				const activationThreshold = windowHeight * 0.4 // 40% from top
				...
				if (firstStep && lastStep) {
					const firstRect =
						firstStep.getBoundingClientRect()
					const lastRect =
						lastStep.getBoundingClientRect()
