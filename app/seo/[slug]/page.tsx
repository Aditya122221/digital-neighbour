...
export async function generateMetadata({
  ...
  const defaultSeoData = await loadSeoPageData("seo");

  // Get base data from Sanity
  const baseHeading =
    defaultSeoData?.metadata ?? defaultSeoData?.hero?.heading ?? "SEO Services";
  ...
  if (requestedSlug === DEFAULT_SEO_SLUG || resolvedKey === DEFAULT_SEO_SLUG) {
...
export default async function SeoSlugPage({
  ...
  const resolveDefaultHeroImage = async () => {
    const defaultSeoData = await defaultSeoDataPromise;
    return (
      defaultSeoData?.hero?.defaultHeroImage ||
      defaultSeoData?.hero?.image ||
      undefined
    );
  };
  ...
  if (!allowedSlugs.includes(resolvedKey)) {
    if (locationSlug) {
      ...
      const baseData = await defaultSeoDataPromise;
      ...
      const defaultHeroImage = await resolveDefaultHeroImage();
      return renderSeoPage(personalizedData, defaultHeroImage);
    }
  ...
  const defaultHeroImage = await resolveDefaultHeroImage();
  return renderSeoPage(currentSeoData, defaultHeroImage);
}

function renderSeoPage(data: SeoPageData, defaultHeroImage?: string | null) {
  return (
    <main>
