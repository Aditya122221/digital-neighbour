...
async function seedCaseStudiesSection(dataDir: string) {
  ...
  try {
    const existing = await retryWithBackoff(async () => {
      return await client.fetch(`*[_type == "caseStudiesSection"][0]`);
    });
...
function transformApartData(rawApartData: any) {
  ...
  return {
    ...
    ours: Array.isArray(rawApartData.ours) ? rawApartData.ours : [],
    others: Array.isArray(rawApartData.others) ? rawApartData.others : [],
  };
}
      ...
      (rawCaseStudies.items || []).map((item: any, index: number) => ({
        ...
        bgImages: addKeys(
          (item.bgImages || []).map((image: any) => ({
            externalUrl: typeof image === "string" ? image : "",
          })),
          "case-image",
        ),
